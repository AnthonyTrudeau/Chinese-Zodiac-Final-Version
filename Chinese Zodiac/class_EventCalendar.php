...
class EventCalendar{
    ...
    public function addEvent($Date, $Title, $Description){
        if ((!empty($Date)) && (!empty($Title))) {
            ...
            if ($QueryResult === False){
                echo "<p>Unable to save th event. Error code " . $this->DBConnect->errno . ": " . $this->DBConnect->error . "</p>\n";
            }
            else{
        ...
        if ($QueryResult !== FALSE) {
            if ($QueryResult->num_rows > 0) {
                    while (($Row = $QueryResult->fetch_assoc()) !== NULL) {
                        echo "<p>Event Title: " . $Row['Title'] . "</p><br />";
    ...
    public function getMonthlyCalendar($Year, $Month){
        ...
        $LeapYearFlag = date('L', $FirstDay); // 1= leap year
        ...
        for ($i = 1; $i <= $LastDay; $i++){
            ...
            if ($QueryResult !== FALSE){
                if ($QueryResult->num_rows > 0) {
                    while (($Row = $QueryResult->fetch_assoc()) !== NULL){
                        echo "<br /><a href='EventDetails.php?" . "PHPSESSIONID=" . session_id() . "&EventID=" . $Row['EventID'] . "'>" . htmlentities($Row['Title']) . "</a>";
                    }
                }
            }
            echo "</td>";
